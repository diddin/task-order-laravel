    ...
    <div class="py-6">
        <div class="max-w-6xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white dark:bg-gray-800 shadow-sm sm:rounded-lg p-6 text-gray-900 dark:text-gray-100">
                ...
                @if($customer->networks && $customer->networks->count())
                    @foreach($customer->networks as $network)
                        <div class="mb-6 border border-gray-600 rounded p-4">
                            ...
                            <a href="{{ route('master.networks.show', $network) }}" class="text-blue-500">Lihat Jaringan</a>

                            {{-- Tasks (Optional) --}}
                            @if($network->tasks && $network->tasks->count())
                                <div class="mt-4">
                                    <h5 class="font-semibold">Daftar Tiket:</h5>
                                    <ul class="list-disc list-inside text-sm">
                                        @foreach($network->tasks as $task)
                                            <li>
                                                <a href="{{ route('master.tasks.show', $task) }}" class="text-blue-400 hover:underline">
                                                    {{ $task->detail }}
                                                </a>
                            ...
                            @else
                                <p class="text-sm text-gray-400 mt-2">Belum ada tiket pada jaringan ini.</p>
                            @endif
                        </div>
                    @endforeach
                @else
                ...
                <div class="mt-6">
                    <a href="{{ route('master.customers.index') }}" class="bg-gray-500 text-white px-4 py-2 rounded hover:bg-gray-600">
                        Kembali
                    </a>
                </div>
            </div>
