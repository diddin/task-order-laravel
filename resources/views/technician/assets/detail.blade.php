        ...
        <div class="wrapper">
            ...
            <form action="{{ route('assets.update', $asset) }}" id="form-asset" method="POST" enctype="multipart/form-data">
                ...
                <div class="item-asset">
                    <div class="img-asset overflow-x-auto whitespace-nowrap flex gap-1 p-0 border rounded" style="max-width: 100%;">
                ...
                @foreach ($portGroups as $group)
                    @php
                        $filteredPorts = $asset->ports->filter(function ($p) use ($group) {
                                    ...
                                    <select name="data_port[]" class="form-select">
                                        @for ($i = $group['start']; $i <= $group['end']; $i++)
                                            ...
                                            <option value="{{ $portValue }}" {{ intval($port['port']) === $portValue ? 'selected' : '' }}>
                                                {{ $portLabel }} ini {{ intval($port['port']) }} beda dengan {{ $portValue}}
                                            </option>
                                        @endfor
                                    </select>
                    ...
                    <div class="input-group">
                        ...
                        <div class="select-grid">
                            <input 
                                ...
                                value="{{ old('patchcore_port_label', optional($asset)->patchcore_port_label) }}"
                            />
                            <input 
                                type="text" 
                                name="patchcore_jumper_label" 
                                value="{{ old('patchcore_jumper_label', optional($asset)->patchcore_jumper_label) }}"
                            />
                        </div>
                    </div>
                </div>
                ...
                <div class="button-bottom">
                    <div class="container">
                        ...
                        <div class="btn-secondary btn-disabled">
                            <a href="{{ route(Auth::user()->role->name . '.taskorders.progress', $task) }}">
                                BATAL
                            </a>
                        </div>
